...
async function startServer() {
  ...
  function evaluateThresholds(v, i, rel) {
    let niveau = "NORMAL";
    ...
    if (v === 0) {
      ...
      niveau = "DANGER";
      message = `Surtension secteur d\xE9tect\xE9e (${v}V > ${settings.maxVoltage}V)`;
  ...
  const handleGetData = (req, res) => {
    ...
    if (!isEspActive && !isSimulate) {
      ...
      if (state.lastEsp32Seen === 0) {
        state.niveau = "NORMAL";
        state.message = "En attente de transmission ESP32 (Wi-Fi d\xE9connect\xE9)";
      } else {
        state.niveau = "ATTENTION";
        state.message = "Signal ESP32 interrompu (Wi-Fi d\xE9connect\xE9)";
      }
    } else if (isSimulate && !isEspActive) {
      simPhase += 0.2;
      const noise = (Math.random() - 0.5) * 0.6;
      const rawV = Number((228 + Math.sin(simPhase * 0.2) * 2 + noise).toFixed(1));
